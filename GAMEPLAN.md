...
- **Notes:** dedicated edge-to-edge, full-viewport OneNote-style workspace with a notebook/section rail, searchable page list, full writing canvas, quick formatting helpers, auto-save through the main account data, pinning, editing, and confirmed deletion. Desktop fills all space between the app header and bottom navigation with independently scrolling columns; mobile stays stacked and scrollable. Existing tags act as notebook sections so older notes migrate without data changes.
...
- **Calendar sync (Sep 24):** devices no longer overwrite each other's calendar. Every add/edit stamps the event's `updatedAt`, every delete writes a tombstone to `dg-cal-deleted` (`{eventId: deletedAt}`), and incoming lists (live sync, including older copies via `subscribe(..., { deliverOlder: true })`, and the load, which also merges this device's saved copy) are merged per event by `mergeCalendarEvents`; when the merge holds something the upload lacked, the merged list is re-uploaded so every device and the reminder function converge. Unstamped (pre-Sep 24) events follow the old rule when only one side has them, so old copies cannot resurrect earlier deletes. The calendar shows a loading state and refuses saves until loaded; "All events" deletes only that series; the grid starts early enough for events before "Day starts at" and draws each event exactly once; repeating events are edited, not dragged; offline deletes save after 3 s with a device safety copy. Tests: `tests/calendar-sync.test.cjs` runs the real storage + calendar code on simulated devices.
...
- **Persistence rule:** all main-data edits must go through `save(current => next)` so rapid actions compose against `dataRef`; calendar edits must go through `saveCalEvents(events => next)`. Never upload `dg-cal-events` any other way: `saveCalEvents` stamps changes and records tombstones, and `writeCalendar` is only for already-merged lists. Never call Firestore directly from a component or put storage side effects inside a React state updater.
...
### Open data-safety findings (audit Sep 24, reproduced with the real code; not fixed yet)
The main document (`daily-grind-v13`: habits, check-offs, tasks, notes, store, settings) still has the whole-document last-writer-wins problem the calendar had: a phone that has not caught up (backgrounded, offline, just opened) can erase check-offs, notes, completions and purchases made on another device. The task ledger only protects a task's existence, not edits/completions, and its live-listener repair can itself upload a stale copy. Fixing this needs per-item merging for the main document (like the calendar now has). Other open items:
- Offline device's queued main-doc write lands after newer data and listeners ignore it (devices drift; next save erases one side). Last-chance note save (`setLocalOnly`) can win over newer cloud data on the next open.
- Day rollover runs lazily and on devices that have not caught up: a daily task completed first thing can bounce back; evening completions can be missed from history, resetting the streak.
- Notes: ink is stored twice, so ~5 handwritten pages fill the 900 KB limit and then every save in the app fails with a vague message; a blocked note save still shows SAVED; restore/import with a page open is undone by the open editor.
- Habits: editing a past day re-scores it with today's habits and points (a won day can turn lost); backfilling a missed day never repairs the streak.
- Sign-in with pre-account device data merges by timestamp only; sign-out discards pending unsynced edits.
